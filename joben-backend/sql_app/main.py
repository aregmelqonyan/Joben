...
# def register_user(user: schemas.CompanyUserCreate, tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
@app.post("/login_company", response_model=schemas.Token)
...
async def send_email(applied_data: schemas.EmailData, tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # message = f"""
    #             <p>Application for {applied_data.job_title} Position</p>
    #             <p>Applicant: {applied_data.first_name} {applied_data.last_name}</p>
    #             <p>Email: {applied_data.user_email}</p>
    #             <p>Contact Info: {applied_data.contact_info}</p>
    #           """
